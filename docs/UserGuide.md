...
**Note**: The existing JSON file on disk will not be overwritten unless you run commands in TrackerGuru, so you must close the TrackerGuru application window to prevent any lost to your data.
</box>
...
A Tag can also optionally contain a **Tag Group** (see [definition](#tag-group)), allowing it to be categorised under a specific group.
